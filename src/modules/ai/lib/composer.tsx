...
export function AiComposerProvider({ children }: ProviderProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("myko:ai-attach-file", onAttach);
    return () => window.removeEventListener("myko:ai-attach-file", onAttach);
  ...
  const submit = () => {
    ...
    // the prompt to the MYKO.md scan template before sending.
    ...
    if (commandSource.startsWith("/") || commandSource.startsWith("#")) {
      ...
      if (outcome.kind === "send-prompt") {
        ...
        if (outcome.commandName) {
          commandMarker = `<myko-command name="${outcome.commandName}" />`;
        }
      }
    }
