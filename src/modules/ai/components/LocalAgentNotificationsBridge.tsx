...
export function LocalAgentNotificationsBridge() {
  ...
  useEffect(() => {
    ...
    if (status === "awaiting-approval") {
      fire("attention", "Myko needs your approval", "Approve a tool to continue");
    } else if (status === "error") {
      fire("error", "Myko run failed", error ?? undefined);
    } else if (status === "idle" && isBusy(was)) {
      fire("finished", "Myko finished", "Your task is ready");
    }
  }, [status, error]);
