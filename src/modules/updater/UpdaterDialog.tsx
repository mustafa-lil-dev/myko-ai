...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S myko-bin";
    case "debian":
      return `sudo apt install ./Myko_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./Myko-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `Myko v${manual.version} is available`
                  : `Myko v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart Myko to finish installing."
              : downloading
                ? progress !== null
