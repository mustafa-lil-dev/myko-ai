...
import type { MykoLspClient } from "./client";
...
async function createSession(
  ...
  const [{ TauriLspTransport }, { MykoLspClient }] = await Promise.all([
    import("./transport"),
    import("./client"),
  ]);

  if (MykoLspClient.hostPid === null) {
    MykoLspClient.hostPid = await invoke<number>("lsp_host_pid").catch(
      () => null,
    );
  }
  ...
  const client = new MykoLspClient({
    transport,
    rootUri,
