# myko-shell-integration (PowerShell)
...
if ($global:__MYKO_HOOKS_LOADED) { return }
$global:__MYKO_HOOKS_LOADED = $true
...
if (Test-Path Function:prompt) {
    Copy-Item Function:prompt Function:__myko_user_prompt -Force -ErrorAction SilentlyContinue
}

function global:__myko_urlencode {
    param([string]$s)
...
function global:__myko_install_readline {
    if ($global:__myko_readline_done) { return }
    if (-not (Test-Path Function:PSConsoleHostReadLine)) { return }
    $global:__myko_readline_done = $true
    ...
    Copy-Item Function:PSConsoleHostReadLine Function:global:__myko_user_readline -Force
    function global:PSConsoleHostReadLine {
        try {
            $line = __myko_user_readline
        } catch {
            # Self-heal: restore the original reader so a broken wrapper can
            # never lock the shell out of input.
            Copy-Item Function:__myko_user_readline Function:global:PSConsoleHostReadLine -Force
            return ''
        }
        try {
            if ($line -is [string] -and $line.Trim().Length -gt 0) {
                $global:__myko_block_seen = $true
                $esc = [char]27
...
function global:prompt {
    ...
    if ($env:MYKO_BLOCKS) {
        ...
        $gap = if ($global:__myko_block_seen) { "`n`n" } else { "`n" }
        return "$oscD$oscA$osc7$gap$oscB"
    }

    $original = if (Test-Path Function:__myko_user_prompt) {
        try { & __myko_user_prompt } catch { "PS $((Get-Location).Path)> " }
    } else {
