...
# zsh, so we shadow $? into `_myko_ret`.

{
  _myko_user_zdotdir="${MYKO_USER_ZDOTDIR:-$HOME}"
  [ -f "$_myko_user_zdotdir/.zshrc" ] && source "$_myko_user_zdotdir/.zshrc"
...
if [[ -z "$__MYKO_HOOKS_LOADED" ]]; then
  __MYKO_HOOKS_LOADED=1
  ...
  _myko_precmd() {
    local _myko_ret=$?
    printf '\e]133;D;%s\e\\' "$_myko_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_myko_urlencode "$PWD")"
  ...
  _myko_preexec() {
    ...
    [[ -n "$MYKO_BLOCKS" ]] && _myko_block_seen=1
    local cmd="${1//[[:cntrl:]]/ }"
    printf '\e]133;C;%s\e\\' "${cmd[1,256]}"
  }

  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _myko_precmd
    add-zsh-hook preexec _myko_preexec
  fi
